<style>

.center {
  display: block;
  margin-left: auto;
  margin-right: auto;
}

</style>

<div class="modal fade" id="detailSiswa" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="detailSiswa" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Detail Siswa</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>   
      <div class="modal-body">
        <input type="hidden" class="form-control" name="id" id="id">

        <div class="row">
          <div class="col-sm-12 align-self-center">
            <div class="form-group">
              <img class="center" src="data:image/jpeg;base64," alt="foto-siswa" width="150" height="150">
              <p class="text-center">Siswa Reguler</p>
            </div>
          </div> 
        </div>
        
        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Nama</label>
              <p style="margin-top:-0.75rem!important">Lasmargo Nendra Wono</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Status Dalam Keluarga</label>
              <p style="margin-top:-0.75rem!important">Anak Kandung</p>
            </div>
          </div>         
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>NIS</label>
              <p style="margin-top:-0.75rem!important">1535010081</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Anak Ke</label>
              <p style="margin-top:-0.75rem!important">1</p>
            </div>
          </div>
         
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Tempat Tanggal Lahir</label>
              <p style="margin-top:-0.75rem!important">11 Maret 2021</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Alamat Peserta Didik</label>
              <p style="margin-top:-0.75rem!important">Jl. Krukah Lama 2/21</p>
            </div>
          </div>
         
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Jenis Kelamin</label>
              <p style="margin-top:-0.75rem!important">Laki-Laki</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>No Tlp Rumah/HP</label>
              <p style="margin-top:-0.75rem!important">085733502462</p>
            </div>
          </div>
         
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Agama</label>
              <p style="margin-top:-0.75rem!important">Islam</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Asal Sekolah</label>
              <p style="margin-top:-0.75rem!important">SDN KRATON</p>
            </div>
          </div>
         
        </div>

        <div class="row">
          <div class="col-sm-6">
          <div class="form-group">
              <label>Pekerjaan Orang Tua</label>
              <p class="ml-2" style="margin-top:-0.75rem!important"><b> a. Ayah :</b> Karyawan Swasta</p><br>
              <p class="ml-2" style="margin-top:-2.75rem!important"><b> b. Ibu :</b> Wiraswasta</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Diterima Di Sekolah Ini</label>
              <p class="ml-2" style="margin-top:-0.75rem!important"><b> - Dikelas :</b> VII-H</p><br>
              <p class="ml-2" style="margin-top:-2.75rem!important"><b> - Pada Tanggal :</b> 16 Juni 2018</p>
            </div>
          </div>
        </div>

        <div class="row" style="margin-top:-1em !important">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Nama Orang Tua</label>
              <p class="ml-2" style="margin-top:-0.75rem!important"><b> a. Ayah :</b> KRISTIONO</p><br>
              <p class="ml-2" style="margin-top:-2.75rem!important"><b> b. Ibu :</b> SUNDARI</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label></label>
              <p style="margin-top:-0.75rem!important"><b>Alamat Orang Tua :</b> Sidomulyo RT 04</p><br>
              <p style="margin-top:-2.75rem!important"><b>No Telepon Rumah :</b> 085733502455</p>
            </div>
          </div>
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Nama Wali Peserta Didik</label>
              <p style="margin-top:-0.75rem!important">-</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Alamat Wali Peserta Didik</label>
              <p style="margin-top:-0.75rem!important">-</p>
            </div>
          </div>         
        </div>

        <div class="row" style="margin-top:-1em !important">
          <div class="col-sm-6">
            <div class="form-group">
              <label>No Telepon Rumah</label>
              <p style="margin-top:-0.75rem!important">-</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Pekerjaan Wali Peserta Didik</label>
              <p style="margin-top:-0.75rem!important">-</p>
            </div>
          </div>         
        </div>
       

        <div class="modal-footer justify-content-end">
          <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
        </div>
      </form>
    </div>
  </div>
</div>