<style>

.center {
  display: block;
  margin-left: auto;
  margin-right: auto;
}

</style>

<div class="modal fade" id="detailSiswa" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="detailSiswa" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Detail Siswa</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>   
      <form>
      <div class="modal-body">
        <input type="hidden" class="form-control" name="id" id="id">
        <div class="row">
          <div class="col-sm-12 align-self-center">
            <div class="form-group">
              <!-- <img class="center" src="data:image/jpeg;base64," alt="foto-siswa" width="150" height="150"> -->
              <img class="center" src="#" id="foto_siswa_v" alt="foto-siswa" width="150" height="150">
              <!-- <p class="text-center">Siswa Reguler</p> -->
            </div>
          </div> 
        </div>
        
        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Nama</label>
              <p style="margin-top:-0.75rem!important" id="nm_siswa_v"> - </p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Status Dalam Keluarga</label>
              <p style="margin-top:-0.75rem!important" id="status_dalam_keluarga_v"> - </p>
            </div>
          </div>         
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>NIS</label>
              <p style="margin-top:-0.75rem!important" id="nis_v"> - </p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Anak Ke</label>
              <p style="margin-top:-0.75rem!important" id="anak_ke_v">-</p>
            </div>
          </div>
         
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Tempat Tanggal Lahir</label>
              <p style="margin-top:-0.75rem!important" id="tempat_lahir_v"> - </p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Alamat Peserta Didik</label>
              <p style="margin-top:-0.75rem!important" id="alamat_v"> - </p>
            </div>
          </div>
         
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Jenis Kelamin</label>
              <p style="margin-top:-0.75rem!important" id="jenis_kelamin_v"> - </p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>No Tlp Rumah/HP</label>
              <p style="margin-top:-0.75rem!important" id="tlp_v">-</p>
            </div>
          </div>
         
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Agama</label>
              <p style="margin-top:-0.75rem!important" id="agama_v"> -</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Asal Sekolah</label>
              <p style="margin-top:-0.75rem!important" id="sekolah_asal_v"> - </p>
            </div>
          </div>
         
        </div>

        <div class="row">
          <div class="col-sm-6">
          <div class="form-group">
              <label>Pekerjaan Orang Tua</label>
              <p class="ml-2" style="margin-top:-0.75rem!important" id="pekerjaan_ayah_v">-</p>
              <br>
              <p class="ml-2" style="margin-top:-2.75rem!important" id="pekerjaan_ibu_v">-</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Diterima Di Sekolah Ini</label>
              <p class="ml-2" style="margin-top:-0.75rem!important" id="diterima_dikelas_v">-</p>
              <br>
              <p class="ml-2" style="margin-top:-2.75rem!important" id="diterima_tgl_v">-</p>
            </div>
          </div>
        </div>

        <div class="row" style="margin-top:-1em !important">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Nama Orang Tua</label>
              <p class="ml-2" style="margin-top:-0.75rem!important" id="nm_ayah_v"> - </p><br>
              <p class="ml-2" style="margin-top:-2.75rem!important" id="nm_ibu_v"> - </p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label></label>
              <p style="margin-top:-0.75rem!important" id="alamat_ortu_v"> - </p><br>
              <p style="margin-top:-2.75rem!important" id="tlp_ortu_v"> -</p>
            </div>
          </div>
        </div>

        <div class="row">
          <div class="col-sm-6">
            <div class="form-group">
              <label>Nama Wali Peserta Didik</label>
              <p style="margin-top:-0.75rem!important" id="nm_wali_v">-</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Alamat Wali Peserta Didik</label>
              <p style="margin-top:-0.75rem!important" id="alamat_wali_v">-</p>
            </div>
          </div>         
        </div>

        <div class="row" style="margin-top:-1em !important">
          <div class="col-sm-6">
            <div class="form-group">
              <label>No Telepon Rumah</label>
              <p style="margin-top:-0.75rem!important" id="tlp_wali_v">-</p>
            </div>
          </div>
          <div class="col-sm-6">
            <div class="form-group">
              <label>Pekerjaan Wali Peserta Didik</label>
              <p style="margin-top:-0.75rem!important" id="pekerjaan_wali_v">-</p>
            </div>
          </div>         
        </div>     

        <div class="modal-footer justify-content-end">
          <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
        </div>
      </form>
    </div>
  </div>
</div>